...
class DbTransfer(object):
    ...
    def pull_db_all_user():
        ...
        cur.execute("SELECT port, u, d, transfer_enable, passwd, switch, enable FROM user where transfer_enable>=1073741824")
        rows = []
        ...
        for row in rows:
            if ServerPool.get_instance().server_is_run(row[0]) is True:
                ...
                if row[5] == 1 and row[6] == 1 and row[1] + row[2] < row[3]:
                    logging.info('db start server at port [%s]' % (row[0]))
